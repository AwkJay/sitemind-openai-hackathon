...
# standards-service/app/retrieval/embeddings_provider.py -> standards-service/
# Relocation note (docs/BUILD_PLAN_CODEBOOK.md step 2): this now loads this
# service's OWN .env (standards-service/.env), not backend/.env — a separate
# process gets a separate env file, same pattern as backend/app/config.py's

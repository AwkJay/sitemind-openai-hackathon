"""Held-out eval for the standalone retrieval package (Phase 3).

Independent of every other eval in this suite (never blended — same
discipline as `run_supply_chain_eval.py`/`run_cost_risk_eval.py`'s pure
function testing pattern, see CLAUDE.md). Three groups of cases:
...
Writes `backend/eval/retrieval_report.json` in the same
`{n_cases, n_pass, accuracy, method, cases}` shape as the rest of the suite.
Run: `python -m eval.run_retrieval_eval` from `backend/` (venv active).
"""

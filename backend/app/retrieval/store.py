"""On-disk persistence for company-uploaded corpora (Phase 3).

Stores each named corpus under `backend/data/company_corpus/<corpus_name>/`
— a NEW data folder, deliberately separate from `backend/data/standards/`
(which this phase never touches, per the hard constraints in
IMPROVEMENTS.md). Chunks are saved as JSON (utf-8, per CLAUDE.md's
mojibake-bug discipline); the dense embedding matrix is saved as a `.npy` —
so a backend restart doesn't lose ingested corpora.
"""
...
# backend/app/retrieval/store.py -> backend/data/company_corpus

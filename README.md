...
- **13 held-out evaluation suites**, each reported separately, never blended into one figure.

## What's REAL vs REPRESENTATIVE

- **Real:** every IS/CEA clause citation (resolves to digitised primary-source text), the
  compliance decision logic, all 13 evals, document ingestion (an uploaded PDF/DOCX is actually
...
13 independent, held-out benchmarks — each measures a different thing (rule-decision accuracy,
...
detection and a recomputed project-finish impact per risk. Each risk includes mitigation options
generated by three specialist agents (procurement alternatives, resequencing/float, and
resource/overtime recovery) — bounded, deterministic computations, not free-form LLM planning.
...
Next.js Command Center  ──REST──>  FastAPI
  (Blueprint design system)          ├─ compliance   (deterministic checks + cited clauses)
...
- `backend/eval/` — labelled test sets and the 13 benchmark runners

...
```
> No `npm` on your machine, only `pnpm`? `pnpm install && pnpm run dev` works too. If
> `node_modules` was previously installed with a different package manager, pnpm may refuse to
> run native postinstall scripts (`ERR_PNPM_IGNORED_BUILDS`) — run `pnpm approve-builds` and
> retry.
